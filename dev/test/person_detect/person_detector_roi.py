...
def write_to_csv(image_name, predictions):
    csv_path = save_dir / 'predictions.csv'
    with open(csv_path, mode='a', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['Image Name', 'Prediction', 'Confidence'] + [f'ROI{i+1}' for i in range(len(predictions))])
        if not csv_path.is_file():
            writer.writeheader()

        data = {'Image Name': image_name, 'Prediction': '', 'Confidence': ''}
        for i, (label, confidence, roi) in enumerate(predictions):
            data[f'ROI{i+1}'] = roi
            data['Prediction'] += f'{label}, '
            data['Confidence'] += f'{confidence}, '

        # Remove trailing commas
        data['Prediction'] = data['Prediction'][:-2]
        data['Confidence'] = data['Confidence'][:-2]
        ...
        for i, det in enumerate(pred):  # per image
            ...
            for *xyxy, conf, cls in reversed(det):
           		...
           		if names[c] == 'person':
           			...
           			if save_csv and len(predictions) > 0:
           				write_to_csv(p.name, predictions)
           			...
           			if save_img or save_crop or view_img:  # Add box to image
           				c = int(cls)  # integer class
           				abel = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
           				#annotator.box_label(xyxy, label, color=colors(c, True))		## print Bounding Box on video
