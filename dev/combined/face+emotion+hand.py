...
class FrameProcessor:
    ...
    def __init__(self, args):
        ...
        self.faces_database = FacesDatabase(args.fg, self.face_identifier,
                                            ...
                                            self.face_detector
                                                if args.run_detector else None, args.no_show)
...
def main():
    """
    Main function, executed by run.sh file.
    """
    ...
    while True:
        ...
        if frame_num == 0:
            ...
            if args.output and\
                not video_writer.open(args.output,
        ...
        if not args.no_show:
            ...
            key = cv2.waitKey(1)
            # # 'x' 키를 누르면 blur_mode를 'NONE'으로 변경
            # if key in {ord('x'), ord('X')}:
            #     blur_mode = "NONE"
            #     print (f"key: {key}")
            #     print(f"blur mod: {blur_mode}")
            # # 'c' 키를 누르면 blur_mode를 'CHANGE'로 변경
            # elif key in {ord('c'), ord('C')}:
            #     blur_mode = "CHANGE"
            #     print (f"key: {key}")
            #     print(f"blur mod: {blur_mode}")
            # # 'O' 키를 누르면 blur_mode를 'DEFAULT'로 변경
