...
class Thread(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.status = True
        self.cap = True
        self.paused = False  # New flag to track the pause state	
        self.cap = True			
    ...
    def run(self):
        ...
        # 웹캠 변경시 해상도 변환 필요
        #self.cap.set(cv2.CAP PROP FOURCC, cv2. VideoWriter fourcc('M','J','P','G'))
        #self.cap.set(cv2.CAP PROP FRAME WIDTH, 2560);		# 2560 -> 3840 변환 필요
        #self.cap.set(cv2.CAP PROP FRAME HEIGHT, 1440);		# 1440 -> 2560 변환 필요
        while self.status:
...
"""        
    def stopVideo(self):
        if self.thread.isRunning():
            self.thread.pause()  # Thread 클래스의 새로운 pause 메서드를 호출합니다.
            self.btn_start.setEnabled(True)
            self.btn_stop.setEnabled(False)
        else:
            self.thread.resume()  # Thread 클래스의 새로운 resume 메서드를 호출합니다.
            self.btn_start.setEnabled(False)
            self.btn_stop.setEnabled(True)    
"""
